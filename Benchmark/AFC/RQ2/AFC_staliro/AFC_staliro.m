...
TU = 0:0.1:10;
U1 = sin(2 * pi * (1 / 5) * TU);  % Example input: sine wave with a period of 5 seconds
U2 = sin(2 * pi * (1 / 3) * TU);
U1 = reshape(U1, [size(TU), 1]);
U2 = reshape(U2, [size(TU), 1]);
% Ensure U has the same length as TU
TU = double(TU);
U1 = double(U1);
U2 = double(U2);
inputTimeseries1 = timeseries(U1, TU);
inputTimeseries2 = timeseries(U2, TU);
...
if table_id==1
    ...
    opt.SampTime = 0.05;
    opt.falsification=1;
    outer_runs = input('Enter number of runs: ');
    opt.runs=outer_runs;
%--------------------------------------------------------------------- 
    if opt_id==1
        cp_array = [1;10]; 
    ...
    opt.optim_params.n_tests = input ('Enter number of tests each run: '); % number of tests
