...
%% add path
    if ismac() == 1
    % MacOS
        addpath(genpath('/Users/quandoan/Documents/MATLAB/AI-CPS-Benchmark/tools/S-TaLiRo/root'));
        addpath(genpath('/Users/quandoan/Documents/MATLAB/AI-CPS-Benchmark/Benchmark/ACC/'));
    elseif isunix() == 1
    % Linux
...
%% set parameters
global cur_mdl agent G_ego t_gap D_default inputTimeseries v_set vmax_ego vmin_ego amin_ego amax_ego x0_lead v0_lead x0_ego v0_ego amin_lead amax_lead use_MPC MPC_a_ego;
...
mdl_list = {'ACC_T_staliro'}; %, 'ACC_RL_hybrid_staliro', 'ACC_RL_hybrid_average_staliro'};
% agent list
agent_list = {'ACC_SAC_Agent_9_11'}; % 'ACC_DDPG_Agent_9_22','ACC_SAC_Agent_9_11','ACC_TD3_Agent_9_11'}; % 'ACC_DDPG_Agent_8_29','ACC_DDPG_Agent_9_11','ACC_DDPG_Agent_9_13'
% hybrid agent list
...
T = 50;
Ts = 0.1;
%Placeholder for inputTimeseries
TU = 0:Ts:T;
...
G_ego = tf(1,[0.5,1,0]);
t_gap = input('Enter value for t_gap: '); % 1.4;
D_default = input('Enter value for D_default: '); %10;
% Specify the driver-set velocity in m/s.
v_set = input('Enter value for v_set: '); % 30;
% ego car minimum/maximum velocity
vmax_ego = input('Enter value for vmax_ego: '); %50;
vmin_ego = input('Enter value for vmin_ego: '); %0;
% the acceleration is constrained to the range [-3,2] (m/s^2).
amin_ego = input('Enter value for amin_ego: '); %-3;
amax_ego = input('Enter value for amax_ego: '); %2;
% the velocity and the position of ego car
x0_ego = input('Enter value for x0_ego: '); %10;
v0_ego = input('Enter value for v0_ego: '); %25;

% the velocity and the position of lead car
x0_lead = input('Enter value for x0_lead: '); %70;
v0_lead = input('Enter value for v0_lead: '); %40;
% the acceleration is constrained to the range [-1,1] (m/s^2).
amin_lead = input('Enter value for amin_lead: '); %-1;
amax_lead = input('Enter value for amax_lead: '); %1;
% RL parameters
use_MPC = 0;
MPC_a_ego = 0;
...
trials = input('Enter number of trials: ');            % # of Runs
max_obj_eval = input('Enter max number of simulations: ');     % Max # of simulations
%% Print arguement list
disp('Ego car:');
disp(['amin_ego: ', num2str(amin_ego)]);
disp(['amax_ego: ', num2str(amax_ego)]);
disp(['v0_ego: ', num2str(v0_ego)]);
disp(['vmin_ego: ', num2str(vmin_ego)]);
disp(['vmax_ego: ', num2str(vmax_ego)]);
disp(['x0_ego: ', num2str(x0_ego)]);
disp(' ');
disp('Lead car:');
disp(['amin_lead: ', num2str(amin_lead)]);
disp(['amax_lead: ', num2str(amax_lead)]);
disp(['v0_lead: ', num2str(v0_lead)]);
disp(['x0_lead: ', num2str(x0_lead)]);
disp(' ');
disp('Simulation stats:');
disp(['D_default:', num2str(D_default)]);
disp(['t_gap:', num2str(t_gap)]);
disp(['v_set:', num2str(v_set)]);
disp(['Number of trials:', num2str(trials)]);
disp(['Max number of simulations:', num2str(max_obj_eval)]);
...
short_results_display(results, history);
%---------------------------------------------------------------------
