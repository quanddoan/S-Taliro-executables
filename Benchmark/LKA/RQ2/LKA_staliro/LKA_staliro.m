...
%% add path
    ...
    % MacOS
        ...
        addpath(genpath('/Users/quandoan/Documents/MATLAB/AI-CPS-Benchmark/Benchmark/LKA'));
    elseif isunix() == 1
    % Linux
...
global T Ts Vx e1_initial e2_initial u_min u_max;
mdl_name = 'LKA';
tool_name = 'staliro';
...
trials = input('Enter number of runs: ');            % # of Runs
...
disp('Enter a range for longitudinal velocity Vx:')
Vx_min = input('Min: ');
Vx_max = input('Max: ');
disp('Enter a range for initial lateral deviation e1_initial: ');
e1_min = input('Min: ');
e1_max = input('Max: ');
disp('Enter a range for intial yaw angle e2_initial: ');
e2_min = input('Min: ');
e2_max = input('Max: ');
...
for solver_index = 1:numel(solver_list)
    ...
    for mdl_index = 1:numel(mdl_list)
        ...
        if strcmp(cur_mdl,'LKA_T_staliro')
            % falsification
            [results, history] = staliro(model,[Vx_min Vx_max;e1_min e1_max;e2_min e2_max],[],[],SP.phi,SP.pred,SP.tf,opt);
            % save
...
disp('Best Roburstness: ')
disp(results.run.bestRob);
disp('Best Cost: ');
disp(results.run.bestCost);
disp('Falsified: ');
disp(results.run.falsified);
disp('Best sample: ');
